...
const getProducts = async (req, res, next) => {
  try {
    const total = await Product.countDocuments(); // Get total number of products
    const maxPageLimit = 12; // Maximum number of products to show per page
    const maxSkip = total === 0 ? 0 : total - 1;
    const limit = Number(req.query.limit) || maxPageLimit;
    ...
    const products = await Product.find({
      ...
      .limit(limit > maxPageLimit ? maxPageLimit : limit)
